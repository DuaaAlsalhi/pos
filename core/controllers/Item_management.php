...
class item_management extends Controller
{
    // RESTful:
    // Get
    // Post
    ...
    {  
        $item = new Item();
        $result =  $item->where('name' , $_POST['itemName']);
    ...
    function update()
    {
        ...
        try {
            ...
            if (!isset($this->request_body['quantity'])) {
                throw new \Exception('item_quantity_out_of_stock');
            }
            if (!is_bool($this->request_body['quantity'])) {
                throw new \Exception('item_quantity_not_bool');
            }
